...
class max_min_doc_est:
    def valida_maxmin_estudiante_asign():
        while True:
    ...
    def valida_maxmin_docente_asign():
        while True:
            print("Hi docente..:")
...
# de la carrera
def valida_maxmin_estud_programa():
    while True:
        anios = int(input('Cantidad de estudiante para aperturar el programa : '))
        if anios < int(reglas_de_negocio["min_estud_programa"])  or anios > int(reglas_de_negocio["max_estud_programa"]):

            print(f'La duracion de la carrera no puede ser menor a {reglas_de_negocio["min_estud_programa"]} o mayor a {reglas_de_negocio["max_estud_programa"]} años')
        else:
...
def valida_duracion_programa():
    while True:
        ...
        if anios < int(reglas_de_negocio["min_estud_curso"])  or anios > int(reglas_de_negocio["max_estud_curso"]):
            ...
            if anios == 5:
                print("Agregar descuentos de 10 % \n")
            elif anios == 4:
                print("Agregar descuentos de 5 % \n")
